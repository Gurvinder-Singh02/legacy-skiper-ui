...
```bash
  npx shadcn@latest add "http://legacy.skiper-ui.com/registry/image-cursortrail.json"
```
...
<Step>Copy the source code to `components/ui/image-cursortrail.tsx` </Step>

<ComponentSource name="image-cursortrail" />
