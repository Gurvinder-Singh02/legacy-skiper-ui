...
```bash
  npx shadcn@latest add "http://legacy.skiper-ui.com/registry/image-ripple.json"
```
...
<Step>Copy the source code to `components/ui/image-ripple.tsx` </Step>

<ComponentSource name="image-ripple" />
...
```tsx
import ImageCursorTrail from "../ui/image-ripple"
```
